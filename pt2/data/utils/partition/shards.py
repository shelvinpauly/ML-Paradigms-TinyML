...
def allocate_shards(
        ori_dataset: Dataset, num_clients: int, num_shards: int, shard_var: int,
    ...
    for i in range(num_clients):
        curr_ns = num_shards
        if shard_var > 0:
            if len(idx_shard) - (num_clients - (i + 1)) + 1 < num_shards + shard_var + 1:
                curr_ns = random.randrange(1, len(idx_shard) - (num_clients - (i + 1)) + 1)
            else:
                curr_ns = random.randrange(num_shards - shard_var, num_shards + shard_var + 1)
        rand_set = random.sample(idx_shard, curr_ns)
